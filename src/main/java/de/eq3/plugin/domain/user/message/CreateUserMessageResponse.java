...
@SystemSentMessageDocumentation(
		messageType = PluginMessageType.Fields.CREATE_USER_MESSAGE_RESPONSE,
		description = "Message sent by the Home Control Unit containing the result of creating a user message to be "
				+ "shown in the Homematic IP smartphone app.",
		envelope = PluginMessage.class,
		correspondingMessage = CreateUserMessageRequest.class)
public class CreateUserMessageResponse implements Body {
	@FieldDocumentation(
			description = "Identifier of the created user message.",
			example = "c0acf4e7-6e23-42cf-ae93-364f65b9e22a",
			nullable = false)
	private String userMessageId;

	@FieldDocumentation(
			description = "Flag to indicate whether the user message was created successfully.",
			example = "true",
			nullable = false)
