...
package de.eq3.plugin.domain.discover;
...
@PluginSentMessageDocumentation(
		messageType = PluginMessageType.Fields.DISCOVER_RESPONSE,
		description = "Message sent by the plugin containing discovered devices available for inclusion in the "
				+ "Homematic IP system.",
		envelope = PluginMessage.class,
		correspondingMessage = DiscoverRequest.class)
public class DiscoverResponse implements Body {
	@FieldDocumentation(
			description = "Flag to indicate whether the device discovery was successful.",
			example = "true",
			nullable = false)
	private boolean success;

	@JsonDeserialize(contentUsing = DeviceDeserializer.class)
	@FieldDocumentation(description = "Set of discovered devices.", nullable = true)
